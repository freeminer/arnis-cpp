...
struct BiomeSample
{
	std::uint8_t land_cover = 0, water_distance = 0;
};
...
std::vector<std::int64_t> pack_chunk_biome_indices(
		const std::array<std::uint8_t, 16> &, unsigned bits);
std::array<std::string, 16> chunk_biome_names(int chunk_x, int chunk_z,
		const ::arnis::Ground *ground, double center_latitude, int world_origin_x = 0,
		int world_origin_z = 0);
ChunkBiomeData build_chunk_biomes(
		int chunk_x, int chunk_z, const BiomeSampler &, Climate, double center_latitude);
