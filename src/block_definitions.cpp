...
{
	std::size_t operator()(
			std::tuple<std::uint8_t, StairFacing, StairShape> const &t) const noexcept
	{
...
static std::unordered_map<std::tuple<std::uint8_t, StairFacing, StairShape>,
		BlockWithProperties, StairKeyHash>
		STAIR_CACHE;
struct Value
{
	static Value String(std::string);
	static Value Compound(std::unordered_map<std::string, Value>);
};
...
Block get_stair_block_for_material(const Block &material)
{
	...
	if (material == STONE || material == COBBLESTONE)
		return COBBLESTONE_STAIRS;
	else if (material == STONE_BRICKS)
		return STONE_BRICK_STAIRS;
	else if (material == MOSSY_COBBLESTONE)
		return MOSSY_COBBLESTONE_STAIRS;
	else if (material == MOSSY_STONE_BRICKS)
		return MOSSY_STONE_BRICK_STAIRS;
	else if (material == CRACKED_STONE_BRICKS)
		return STONE_BRICK_STAIRS;
	else if (material == TUFF)
		return COBBLESTONE_STAIRS;
	else if (material == ANDESITE)
		return ANDESITE_STAIRS;
	else if (material == DIORITE || material == POLISHED_DIORITE)
		return POLISHED_DIORITE_STAIRS;
	else if (material == DEEPSLATE || material == DEEPSLATE_BRICKS ||
			 material == COBBLED_DEEPSLATE)
		return DEEPSLATE_BRICK_STAIRS;
	else if (material == POLISHED_DEEPSLATE)
		return POLISHED_DEEPSLATE_STAIRS;
	else if (material == MUD_BRICKS || material == MUD)
		return MUD_BRICK_STAIRS;
	else if (material == OAK_PLANKS)
		return OAK_STAIRS;
	else if (material == SPRUCE_PLANKS || material == SPRUCE_LOG)
		return SPRUCE_STAIRS;
	else if (material == DARK_OAK_PLANKS || material == DARK_OAK_LOG)
		return DARK_OAK_STAIRS;
	else if (material == OAK_LOG)
		return OAK_STAIRS;
	else if (material == POLISHED_ANDESITE)
		return POLISHED_ANDESITE_STAIRS;
	else if (material == SMOOTH_STONE)
		return POLISHED_ANDESITE_STAIRS;
	else if (material == CHISELED_STONE_BRICKS)
		return STONE_BRICK_STAIRS;
	else if (material == BLACK_TERRACOTTA)
		return POLISHED_BLACKSTONE_BRICK_STAIRS;
	else if (material == BLACKSTONE)
		return POLISHED_BLACKSTONE_BRICK_STAIRS;
	else if (material == BLUE_TERRACOTTA || material == CYAN_TERRACOTTA)
		return DEEPSLATE_BRICK_STAIRS;
	else if (material == BRICK)
		return BRICK_STAIRS;
	else if (material == BROWN_CONCRETE)
		return MUD_BRICK_STAIRS;
	else if (material == BROWN_TERRACOTTA)
		return MUD_BRICK_STAIRS;
	else if (material == END_STONE_BRICKS)
		return END_STONE_BRICK_STAIRS;
	else if (material == GRAY_CONCRETE)
		return POLISHED_BLACKSTONE_BRICK_STAIRS;
	else if (material == GRAY_TERRACOTTA)
		return MUD_BRICK_STAIRS;
	else if (material == LIGHT_BLUE_TERRACOTTA)
		return POLISHED_DIORITE_STAIRS;
		...
		return POLISHED_BLACKSTONE_BRICK_STAIRS;
	else if (material == POLISHED_BLACKSTONE_BRICKS)
		return POLISHED_BLACKSTONE_BRICK_STAIRS;
	else if (material == POLISHED_GRANITE)
		return POLISHED_GRANITE_STAIRS;
	else if (material == QUARTZ_BLOCK)
		return POLISHED_DIORITE_STAIRS;
	else if (material == QUARTZ_BRICKS)
		return POLISHED_DIORITE_STAIRS;
	else if (material == SMOOTH_QUARTZ || material == GLASS)
		return QUARTZ_STAIRS;
	else if (material == SANDSTONE)
		return SMOOTH_SANDSTONE_STAIRS;
	else if (material == SMOOTH_SANDSTONE)
		return SMOOTH_SANDSTONE_STAIRS;
	else if (material == WHITE_CONCRETE)
		return QUARTZ_STAIRS;
	else if (material == WHITE_TERRACOTTA)
		return QUARTZ_STAIRS;
	else if (material == RED_NETHER_BRICKS)
		return RED_NETHER_BRICK_STAIRS;
	else if (material == WAXED_OXIDIZED_COPPER)
		return WAXED_OXIDIZED_CUT_COPPER_STAIRS;
	else if (material == WAXED_COPPER_BLOCK)
		return WAXED_CUT_COPPER_STAIRS;
	else if (material == WAXED_EXPOSED_COPPER ||
			 material == WAXED_EXPOSED_CHISELED_COPPER ||
			 material == WAXED_EXPOSED_CUT_COPPER)
		return WAXED_EXPOSED_CUT_COPPER_STAIRS;
	else if (material == IRON_BLOCK || material == NETHERITE_BLOCK)
		return POLISHED_BLACKSTONE_BRICK_STAIRS;
	else if (material == HAY_BALE)
		return OAK_STAIRS;
	else if (material == GRAVEL)
		return COBBLESTONE_STAIRS;
	else if (material == GRASS_BLOCK || material == MOSS_BLOCK)
		return MOSSY_COBBLESTONE_STAIRS;
	else
		return STONE_BRICK_STAIRS;
}
...
Block get_slab_block_for_material(const Block &material)
{
	...
	if (material == OAK_PLANKS || material == OAK_LOG || material == SPRUCE_PLANKS ||
...
static const std::array<Block *, 10> WINDOW_VARIATIONS = {&GLASS,

		&GRAY_STAINED_GLASS, &LIGHT_GRAY_STAINED_GLASS, &GRAY_STAINED_GLASS,
		&BROWN_STAINED_GLASS, &WHITE_STAINED_GLASS, &TINTED_GLASS,
		&LIGHT_BLUE_STAINED_GLASS, &CYAN_STAINED_GLASS, &BLUE_STAINED_GLASS
...
static const std::vector<std::pair<RGB, std::vector<Block *>>> DEFINED_COLORS = {
		...
		{{186, 195, 142},
				{&END_STONE_BRICKS, &SANDSTONE, &SMOOTH_SANDSTONE, &LIGHT_GRAY_CONCRETE}},
		...
		{{159, 82, 36}, {&BROWN_TERRACOTTA, &BRICK, &POLISHED_GRANITE, &BROWN_CONCRETE,
								&NETHERITE_BLOCK, &POLISHED_DEEPSLATE}},
		{{128, 128, 128},
				{&POLISHED_ANDESITE, &LIGHT_GRAY_CONCRETE, &SMOOTH_STONE, &STONE_BRICKS}},
		{{174, 173, 174},
				{&POLISHED_ANDESITE, &LIGHT_GRAY_CONCRETE, &SMOOTH_STONE, &STONE_BRICKS}},
		{{141, 101, 142}, {&STONE_BRICKS, &BRICK, &MUD_BRICKS}},
		{{142, 60, 46},
				{&BLACK_TERRACOTTA, &NETHERITE_BLOCK, &NETHER_BRICK, &POLISHED_GRANITE,
						&POLISHED_DEEPSLATE, &BROWN_TERRACOTTA}},
		{{153, 83, 28}, {&BLACK_TERRACOTTA, &POLISHED_GRANITE, &BROWN_CONCRETE,
								&BROWN_TERRACOTTA, &STONE_BRICKS}},
		{{224, 216, 175}, {&SMOOTH_SANDSTONE, &LIGHT_GRAY_CONCRETE, &POLISHED_ANDESITE,
								  &SMOOTH_STONE}},
		{{188, 182, 179}, {&SMOOTH_SANDSTONE, &LIGHT_GRAY_CONCRETE, &QUARTZ_BRICKS,
								  &POLISHED_ANDESITE, &SMOOTH_STONE}},
		{{35, 86, 85},
				{&POLISHED_BLACKSTONE_BRICKS, &BLUE_TERRACOTTA, &LIGHT_BLUE_TERRACOTTA}},
		{{255, 255, 255}, {&WHITE_CONCRETE, &QUARTZ_BRICKS, &QUARTZ_BLOCK}},
		{{209, 177, 161}, {&WHITE_TERRACOTTA, &SMOOTH_SANDSTONE, &SMOOTH_STONE,
								  &SANDSTONE, &LIGHT_GRAY_CONCRETE}},
		{{191, 147, 42}, {&SMOOTH_SANDSTONE, &SANDSTONE, &SMOOTH_STONE}},
		{{36, 137, 199}, {&LIGHT_BLUE_CONCRETE}},
		{{45, 47, 143}, {&BLUE_CONCRETE, &BLUE_TERRACOTTA}},
		{{21, 119, 136}, {&CYAN_CONCRETE}}, {{73, 91, 36}, {&GREEN_CONCRETE}},
		{{120, 200, 30}, {&LIME_CONCRETE}}, {{241, 175, 21}, {&YELLOW_CONCRETE}},
		{{186, 133, 35}, {&YELLOW_TERRACOTTA}}, {{200, 30, 30}, {&RED_CONCRETE}},
...
{
	std::mt19937 &rng = global_rng();
	if (building_type == "residential" || building_type == "house" ||
			building_type == "apartment") {
		const std::vector<Block> residential_windows = {GLASS, WHITE_STAINED_GLASS,
				LIGHT_GRAY_STAINED_GLASS, BROWN_STAINED_GLASS};
		std::uniform_int_distribution<std::size_t> dist(
				0, residential_windows.size() - 1);
	...
	if (building_type == "hospital" || building_type == "school" ||
		...
		std::uniform_int_distribution<std::size_t> dist(
				0, institutional_windows.size() - 1);
		return institutional_windows[dist(rng)];
	}
	if (building_type == "hotel" || building_type == "restaurant") {
		const std::vector<Block> hospitality_windows = {GLASS, WHITE_STAINED_GLASS};
		std::uniform_int_distribution<std::size_t> dist(
				0, hospitality_windows.size() - 1);
	...
	if (building_type == "industrial" || building_type == "warehouse") {
		const std::vector<Block> industrial_windows = {
				GLASS, GRAY_STAINED_GLASS, LIGHT_GRAY_STAINED_GLASS, BROWN_STAINED_GLASS};
...
{
	const std::vector<Block> floor_options = {WHITE_CONCRETE, GRAY_CONCRETE,
			LIGHT_GRAY_CONCRETE, POLISHED_ANDESITE, SMOOTH_STONE, STONE_BRICKS,
			MUD_BRICKS, OAK_PLANKS};
	std::mt19937 &rng = global_rng();
...
Block get_random_floor_block(ChaCha8Rng &rng)
{
	...
	const std::vector<Block> floor_options = {WHITE_CONCRETE, GRAY_CONCRETE,
			LIGHT_GRAY_CONCRETE, POLISHED_ANDESITE, SMOOTH_STONE, STONE_BRICKS,
			MUD_BRICKS, OAK_PLANKS};
	return floor_options[rng.uniform(static_cast<std::uint32_t>(floor_options.size()))];
}
...
Block get_building_wall_block_for_color(const RGB &color)
{
	...
	auto best_it = std::min_element(DEFINED_COLORS.begin(), DEFINED_COLORS.end(),
			[&color](const std::pair<RGB, std::vector<Block *>> &a,
					const std::pair<RGB, std::vector<Block *>> &b) {
				return colors::rgb_distance(color, a.first) <
					   colors::rgb_distance(color, b.first);
			});
...
Block get_fallback_building_block()
{
	...
	const std::vector<Block> fallback_options = {BLACKSTONE, BLACK_TERRACOTTA, BRICK,
			BROWN_CONCRETE, BROWN_TERRACOTTA, DEEPSLATE_BRICKS, END_STONE_BRICKS,
			GRAY_CONCRETE, GRAY_TERRACOTTA, LIGHT_BLUE_TERRACOTTA, LIGHT_GRAY_CONCRETE,
			MUD_BRICKS, NETHER_BRICK, POLISHED_ANDESITE, POLISHED_BLACKSTONE,
			...
			STONE_BRICKS, WHITE_CONCRETE, WHITE_TERRACOTTA, OAK_PLANKS};
	std::mt19937 &rng = global_rng();
...
{
	const std::vector<Block> options = {BLACKSTONE, BLACK_TERRACOTTA, BRICK,
			BROWN_CONCRETE, BROWN_TERRACOTTA, DEEPSLATE_BRICKS, END_STONE_BRICKS,
			GRAY_CONCRETE, GRAY_TERRACOTTA, LIGHT_BLUE_TERRACOTTA, LIGHT_GRAY_CONCRETE,
			MUD_BRICKS, NETHER_BRICK, POLISHED_ANDESITE, POLISHED_BLACKSTONE,
			POLISHED_BLACKSTONE_BRICKS, POLISHED_DEEPSLATE, POLISHED_GRANITE,
...
Block get_castle_wall_block()
{
	...
	const std::vector<Block> castle_wall_options = {STONE_BRICKS, CHISELED_STONE_BRICKS,
			CRACKED_STONE_BRICKS,

			COBBLESTONE, MOSSY_COBBLESTONE, DEEPSLATE_BRICKS, POLISHED_ANDESITE, ANDESITE,
...
{
	const std::vector<Block> options = {STONE_BRICKS, CHISELED_STONE_BRICKS,
			CRACKED_STONE_BRICKS, COBBLESTONE, MOSSY_COBBLESTONE, DEEPSLATE_BRICKS,
			POLISHED_ANDESITE, ANDESITE, SMOOTH_STONE, BRICK};
	return options[rng.uniform(static_cast<std::uint32_t>(options.size()))];
}
