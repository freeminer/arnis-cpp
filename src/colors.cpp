...
{
	auto lab=[](const RGBTuple &c) {
		auto linear=[](std::uint8_t v) { const float x=float(v)/255.f; return x<=.04045f?x/12.92f:std::pow((x+.055f)/1.055f,2.4f); };
		const float r=linear(std::get<0>(c)),g=linear(std::get<1>(c)),b=linear(std::get<2>(c));
		...
		return std::array<float,3>{.21045426f*l+.7936178f*m-.004072047f*s,
				1.9779985f*l-2.4285922f*m+.4505937f*s,
				.025904037f*l+.78277177f*m-.80867577f*s};
	};
	const auto a=lab(from),b=lab(to); const float dl=a[0]-b[0],da=a[1]-b[1],db=a[2]-b[2];
...
std::optional<RGBTuple> color_name_to_rgb_tuple(std::string_view text)
{
	...
	for (unsigned char c : text)
		if (!std::isspace(c) && c != '_' && c != '-') normalized.push_back(char(std::tolower(c)));
	text = normalized;
	...
	if (text == "darkgray" || text == "darkgrey") return RGBTuple{96,96,96};
	if (text == "darkbrown") return RGBTuple{90,50,20};
	if (text == "darkred") return RGBTuple{139,0,0};
	if (text == "dimgray" || text == "dimgrey") return RGBTuple{105,105,105};
	if (text == "firebrick") return RGBTuple{178,34,34};
	...
	if (text == "ivory") return RGBTuple{255,255,240};
	if (text == "khaki") return RGBTuple{240,230,140};
	if (text == "lightblue") return RGBTuple{173,216,230};
	if (text == "lightgray" || text == "lightgrey") return RGBTuple{211,211,211};
	if (text == "lightgreen") return RGBTuple{144,238,144};
	if (text == "lightyellow") return RGBTuple{255,255,224};
	if (text == "lime")
		return std::make_optional(
				std::make_tuple<std::uint8_t, std::uint8_t, std::uint8_t>(0, 255, 0));
	if (text == "limestone") return RGBTuple{246,240,208};
	if (text == "maroon")
		return std::make_optional(
	...
	if (text == "pink") return RGBTuple{255,192,203};
	if (text == "purple")
		return std::make_optional(
	...
	if (text == "salmon") return RGBTuple{250,128,114};
	if (text == "sandstone") return RGBTuple{215,188,138};
	if (text == "silver")
		return std::make_optional(
				std::make_tuple<std::uint8_t, std::uint8_t, std::uint8_t>(192, 192, 192));
	if (text == "tan") return RGBTuple{210,180,140};
	if (text == "teal")
		return std::make_optional(
				std::make_tuple<std::uint8_t, std::uint8_t, std::uint8_t>(0, 128, 0));
	if (text == "white")
