...
BridgeStyle resolve_bridge_style_from_pair(
		const std::optional<std::string> &structure, const std::optional<std::string> &bridge)
{
	if (structure) {
		if (*structure == "arch")
...
bool point_in_polygon(int x, int z, const std::vector<std::pair<int, int>> &poly)
{
	...
	for (std::size_t i = 0; i < n; ++i) {
		...
		const bool intersects = ((zi > zf) != (zj > zf)) &&
				xf < (xj - xi) * (zf - zi) / (zj - zi + std::numeric_limits<double>::epsilon()) + xi;
		if (intersects)
			inside = !inside;
		j = i;
	}
...
std::pair<std::size_t, std::size_t> arch_segment(std::size_t tds, std::size_t total)
{
	if (total < 2)
		return {0, total};
	const std::size_t n_arches = std::max<std::size_t>(1, (total + ARCH_SPAN / 2) / ARCH_SPAN);
...
void place_pillar(WorldEditor &editor, int x, int deck_y, int z, Block body,
		bool with_base)
{
	...
	for (int bx = -1; bx <= 1; ++bx) {
		for (int bz = -1; bz <= 1; ++bz)
			editor.set_block_absolute(body, x + bx, ground_y, z + bz, std::nullopt, std::nullopt);
	}
}
		...
		int centerline_ground_y, std::size_t tds, std::size_t total, bool use_absolute_y)
{
	...
	if (dist_to_deck <= 0)
		return;
	const int max_rise = static_cast<int>(static_cast<float>(dist_to_deck) * ARCH_RISE_FRACTION);
	const float t = arch_local_t(tds, total);
	const int rise_at_cell = static_cast<int>(static_cast<float>(max_rise) * 4.0f * t * (1.0f - t));
...
{
	const float rail_dist = static_cast<float>(block_range) * (std::abs(px) + std::abs(pz)) + 1.0f;
...
{
	return std::min<std::size_t>(2 + total / SUSPENSION_INTER_PYLON_SPACING, SUSPENSION_MAX_PYLONS);
}

int cable_stayed_tower_height(std::size_t total)
{
	const int extra = static_cast<int>(total / CABLE_STAYED_TOWER_HEIGHT_DIVISOR);
	return std::min(CABLE_STAYED_TOWER_BASE_HEIGHT + extra, CABLE_STAYED_TOWER_MAX_HEIGHT);
}
...
{
	for (const auto &[cx, cy, cz] : bresenham_line(left.first, top_y, left.second,
	...
	for (const auto &[cx, cy, cz] : bresenham_line(x1, y1, z1, x2, y2, z2)) {
		...
		if (prev) {
			...
			if (axes_changed >= 2) {
				...
				if (axes_changed == 3)
					editor.set_block_absolute(chain, cx, py, pz, std::nullopt, std::nullopt);
			}
		}
		...
		int block_range, bool start_is_boundary, bool end_is_boundary)
{
	...
	for (std::size_t tds = 0; tds < path.size(); ++tds) {
		...
		if ((tds == 0 && start_is_boundary) || (tds == last && end_is_boundary))
			continue;
		const auto [left, right] = side_offsets(s.x, s.z, s.perp_x, s.perp_z, block_range);
		const int top_y = s.y + 1 + TRUSS_TOP_HEIGHT;
		editor.set_block_absolute(IRON_BLOCK, left.first, s.y + 1, left.second, std::nullopt, std::nullopt);
		editor.set_block_absolute(IRON_BLOCK, right.first, s.y + 1, right.second, std::nullopt, std::nullopt);
		editor.set_block_absolute(IRON_BLOCK, left.first, top_y, left.second, std::nullopt, std::nullopt);
		editor.set_block_absolute(IRON_BLOCK, right.first, top_y, right.second, std::nullopt, std::nullopt);
		if (tds % TRUSS_POST_INTERVAL == 0) {
			for (int h = 1; h <= TRUSS_TOP_HEIGHT; ++h) {
				editor.set_block_absolute(IRON_BLOCK, left.first, s.y + 1 + h, left.second, std::nullopt, std::nullopt);
				editor.set_block_absolute(IRON_BLOCK, right.first, s.y + 1 + h, right.second, std::nullopt, std::nullopt);
			}
		}
		...
		editor.set_block_absolute(IRON_BLOCK, left.first, diag_y, left.second, std::nullopt, std::nullopt);
		editor.set_block_absolute(IRON_BLOCK, right.first, diag_y, right.second, std::nullopt, std::nullopt);
		if (tds % TRUSS_PORTAL_INTERVAL == 0) {
			for (const auto &[bx, by, bz] : bresenham_line(left.first, top_y, left.second,
						 right.first, top_y, right.second))
				editor.set_block_absolute(IRON_BLOCK, bx, by, bz, std::nullopt, std::nullopt);
		}
	}
}
		...
		int block_range, bool start_is_boundary, bool end_is_boundary)
{
	...
	const std::size_t inset = std::max<std::size_t>(
			static_cast<std::size_t>(static_cast<float>(total) * SUSPENSION_TOWER_INSET_FRAC), 2);
	if (inset * 2 + 2 > total)
		return;
	...
	for (std::size_t i = 0; i < n_pylons; ++i)
		pylons.push_back(first + (last - first) * i / std::max<std::size_t>(1, n_pylons - 1));
	...
	for (std::size_t wi = 1; wi < pylons.size(); ++wi) {
		...
		for (std::size_t tds = a; tds <= b; ++tds) {
			...
			const float base_y = static_cast<float>(top_a) + static_cast<float>(top_b - top_a) * t;
			const int cable_y = static_cast<int>(std::round(base_y - dip * 4.0f * t * (1.0f - t)));
			const Block chain = std::abs(s.perp_x) > std::abs(s.perp_z) ? CHAIN_Z : CHAIN_X;
			editor.set_block_absolute(chain, left.first, cable_y, left.second, std::nullopt, std::nullopt);
			editor.set_block_absolute(chain, right.first, cable_y, right.second, std::nullopt, std::nullopt);
			if ((tds - a) % SUSPENSION_HANGER_INTERVAL == 0 && tds != a && tds != b) {
				for (int hy = s.y + 2; hy < cable_y; ++hy) {
					editor.set_block_absolute(IRON_BARS, left.first, hy, left.second, std::nullopt, std::nullopt);
					editor.set_block_absolute(IRON_BARS, right.first, hy, right.second, std::nullopt, std::nullopt);
				}
			}
		}
	}
	...
	const auto [ls, rs] = side_offsets(path[0].x, path[0].z,
			path[0].perp_x, path[0].perp_z, block_range);
	draw_cable(editor, lf.first, path[first_p].y + height, lf.second,
			ls.first, path[0].y + 1, ls.second);
	draw_cable(editor, rf.first, path[first_p].y + height, rf.second,
			rs.first, path[0].y + 1, rs.second);
	...
	draw_cable(editor, ll.first, path[last_p].y + height, ll.second,
			le.first, path[last_idx].y + 1, le.second);
	draw_cable(editor, rl.first, path[last_p].y + height, rl.second,
			re.first, path[last_idx].y + 1, re.second);
}
		...
		int block_range)
{
	...
	const std::vector<std::size_t> pylons = total >= CABLE_STAYED_TWIN_PYLON_LENGTH ?
	...
	for (std::size_t idx = 0; idx < pylons.size(); ++idx) {
		...
		const auto [left_t, right_t] = side_offsets(tower.x, tower.z,
				tower.perp_x, tower.perp_z, block_range);
		...
		const std::size_t anchor_hi = pylons.size() == 1 ? total : (idx == 0 ? split : total);
		for (std::size_t tds = anchor_lo + CABLE_STAYED_ANCHOR_INTERVAL;
				tds < anchor_hi; tds += CABLE_STAYED_ANCHOR_INTERVAL) {
			...
			const auto [left_a, right_a] = side_offsets(anchor.x, anchor.z,
					anchor.perp_x, anchor.perp_z, block_range);
			draw_cable(editor, left_t.first, top_y, left_t.second,
					left_a.first, anchor.y + 1, left_a.second);
			draw_cable(editor, right_t.first, top_y, right_t.second,
					right_a.first, anchor.y + 1, right_a.second);
		}
	}
}
		...
		int block_range, bool start_is_boundary, bool end_is_boundary)
{
	...
	for (std::size_t tds = 0; tds < path.size(); ++tds) {
		...
		for (int h = 1; h <= COVERED_WALL_HEIGHT; ++h) {
			const Block b = h == 2 && tds % COVERED_WINDOW_INTERVAL == 0 ? GLASS : DARK_OAK_PLANKS;
			editor.set_block_absolute(b, left.first, s.y + h, left.second, std::nullopt, std::nullopt);
			editor.set_block_absolute(b, right.first, s.y + h, right.second, std::nullopt, std::nullopt);
		}
		const int roof_y = s.y + COVERED_WALL_HEIGHT + 1;
		const int extent = block_range + 1;
		for (int offset = -extent; offset <= extent; ++offset) {
			const int rx = static_cast<int>(std::round(static_cast<float>(s.x) + s.perp_x * static_cast<float>(offset)));
			const int rz = static_cast<int>(std::round(static_cast<float>(s.z) + s.perp_z * static_cast<float>(offset)));
			editor.set_block_absolute(DARK_OAK_PLANKS, rx, roof_y, rz, std::nullopt, std::nullopt);
		}
	}
}
...
{
	return style == BridgeStyle::Boardwalk ? BOARDWALK_POST_INTERVAL : BEAM_PILLAR_INTERVAL;
}
...
BridgeOutlineIndex BridgeOutlineIndex::build(const std::vector<ProcessedElement> &elements)
{
	...
	for (const auto &elem : elements) {
		...
		if (const auto it = way.tags.find("bridge"); it != way.tags.end())
			bridge = it->second;
		const bool has_style = structure.has_value() || (bridge && (*bridge == "viaduct" ||
				*bridge == "covered" || *bridge == "boardwalk" || *bridge == "cable-stayed" ||
...
std::optional<BridgeStyle> BridgeOutlineIndex::style_for_way(const ProcessedWay &way) const
{
	if (entries_.empty() || way.nodes.empty())
		return std::nullopt;
	const auto [cx, cz] = centroid_xz(way);
	for (const auto &entry : entries_) {
		if (cx < entry.bbox_min_x || cx > entry.bbox_max_x ||
				cz < entry.bbox_min_z || cz > entry.bbox_max_z)
			continue;
		...
		bool is_pillar_position)
{
	if (style == BridgeStyle::Arch) {
		place_arch_spandrel_cell(editor, set_x, cell_y, set_z, centerline_ground_y,
				tds, total, use_absolute_y);
		if (is_centerline) {
			const auto [start, span] = arch_segment(tds, total);
		...
		bool start_is_boundary, bool end_is_boundary)
{
	...
	case BridgeStyle::Truss:
		...
		break;
	case BridgeStyle::Suspension:
		decorate_suspension(editor, path, block_range, start_is_boundary, end_is_boundary);
		break;
	case BridgeStyle::CableStayed:
