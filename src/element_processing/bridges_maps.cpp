...
std::vector<std::pair<int, int>> centerline_samples(const ProcessedWay &way)
{
    ...
    if (last >= 2) {
        ...
        if (take > 0) {
            ...
            for (std::size_t idx = 1; idx < last && out.size() < CENTERLINE_SAMPLE_LIMIT + 2;
                    idx += step) {
...
bool are_dual_carriageway_pair(const ProcessedWay &a, const ProcessedWay &b)
{
    ...
    if (diff > 180.0f)
        diff = 360.0f - diff;
    const bool parallel = diff <= DUAL_CARRIAGEWAY_HEADING_TOLERANCE_DEG ||
...
std::optional<int> decide_internal_ramp(const std::pair<int, int> &xz, int deck_y,
        const std::unordered_map<std::pair<int, int>, std::size_t, XZPairHash> &endpoint_counts,
        const std::unordered_map<std::pair<int, int>, bool, XZPairHash> &boundary_with_external_ramp,
        ...
        std::size_t tds, std::size_t total_bresenham, std::size_t ramp_length) const
{
    ...
    if (start_internal_ramp && tds < ramp_length) {
        ...
        return static_cast<int>(std::round(*start_internal_ramp +
                (deck_y - *start_internal_ramp) * t));
    }
    ...
    if (end_internal_ramp && dist_from_end < ramp_length) {
        ...
        return static_cast<int>(std::round(*end_internal_ramp +
                (deck_y - *end_internal_ramp) * t));
    }
    return deck_y;
}
    ...
    for (const auto &group_entry : groups) {
        ...
        for (const auto idx : group_indices) {
            for (const auto sample : centerline_samples(*bridge_ways[idx]))
                terrain_samples.push_back(editor.get_ground_level(sample.first, sample.second));
        }
        ...
        std::unordered_map<std::pair<int, int>, bool, XZPairHash> boundary_with_external_ramp;
        for (const auto &entry : endpoint_counts) {
            ...
            for (const auto oi : other_it->second) {
                ...
                if (claimed_ramp_ways.contains(candidate.id))
                    continue;
                const bool bridge_side_at_start =
                        candidate.nodes.front().x == xz.first &&
                        candidate.nodes.front().z == xz.second;
                const auto &far_node = bridge_side_at_start ?
                        candidate.nodes.back() : candidate.nodes.front();
        ...
        for (const auto idx : group_indices) {
            ...
            info.end_internal_ramp = decide_internal_ramp(
                    {e.x, e.z}, deck_y, endpoint_counts, boundary_with_external_ramp, editor);
        ...
        const BridgeStructureMap &structures, double scale)
{
    ...
    for (const auto &element : elements) {
        ...
        for (std::size_t i = 1; i < way.nodes.size(); ++i) {
            ...
            for (std::size_t point_idx = skip_first; point_idx < points.size(); ++point_idx) {
                const auto &p = points[point_idx];
                const int x = std::get<0>(p);
                const int z = std::get<2>(p);
                const int cell_y = info ?
