...
void generate_fire_hydrant(WorldEditor &editor, const ProcessedNode &node) {
    ...
    if (hydrant_type == "underground" || hydrant_type == "wall" || hydrant_type == "pond") {
        return;
    }

    // Simple hydrant: brick wall with redstone block on top
    editor.set_block(BRICK_WALL, x, 1, z, std::nullopt, std::nullopt);
    editor.set_block(REDSTONE_BLOCK, x, 2, z, std::nullopt, std::nullopt);
}
