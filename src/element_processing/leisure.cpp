		...
		const bridges::BridgeSurfaceMap &bridge_surface)
{
	...
	if (leisure_it != element.tags.end()) {
		...
		if (leisure_type == "park" || leisure_type == "nature_reserve" ||
				leisure_type == "garden" || leisure_type == "disc_golf_course" ||
				leisure_type == "golf_course") {
			...
			block_type = BLACK_CONCRETE;
				   ...
				   leisure_type == "pitch" || leisure_type == "beach_resort" ||
				   leisure_type == "dog_park") {
		...
		if (auto surf_it = element.tags.find(std::string("surface"));
				surf_it != element.tags.end()) {
			if (const auto *surface_blocks =
							surfaces::get_blocks_for_surface(surf_it->second);
				...
				for (const std::tuple<int, int, int> &t : bresenham_points) {
					...
					editor.set_block(block_type, bx, 0, bz,
							std::optional<std::vector<Block>>({GRASS_BLOCK, STONE_BRICKS,
									SMOOTH_STONE, LIGHT_GRAY_CONCRETE, COBBLESTONE,
									GRAY_CONCRETE}),
							std::nullopt);
				}
		...
		if (corner_addup != std::make_tuple(0, 0, 0)) {
			// Match Rust: a single deterministic stream is consumed only when the
			// closed area is actually processed.
			...
			for (const std::pair<int, int> &p : filled_area) {
				...
				if (leisure_type == "playground" || leisure_type == "recreation_ground") {
					int rc = static_cast<int>(rng.uniform(5000));

					if (rc >= 0 && rc < 10) {
						// Swing set
						for (int y = 1; y <= 3; ++y) {
							editor.set_block(
									OAK_FENCE, x - 1, y, z, std::nullopt, std::nullopt);
							editor.set_block(
									OAK_FENCE, x + 1, y, z, std::nullopt, std::nullopt);
						}
						editor.set_block(
								OAK_PLANKS, x - 1, 4, z, std::nullopt, std::nullopt);
						editor.set_block(OAK_SLAB, x, 4, z, std::nullopt, std::nullopt);
						editor.set_block(
								OAK_PLANKS, x + 1, 4, z, std::nullopt, std::nullopt);
						editor.set_block(
								STONE_BLOCK_SLAB, x, 2, z, std::nullopt, std::nullopt);
					} else if (rc >= 10 && rc < 20) {
						// Slide
						editor.set_block(OAK_SLAB, x, 1, z, std::nullopt, std::nullopt);
						editor.set_block(
								OAK_SLAB, x + 1, 2, z, std::nullopt, std::nullopt);
						editor.set_block(
								OAK_SLAB, x + 2, 3, z, std::nullopt, std::nullopt);

						editor.set_block(
								OAK_PLANKS, x + 2, 2, z, std::nullopt, std::nullopt);
						editor.set_block(
								OAK_PLANKS, x + 2, 1, z, std::nullopt, std::nullopt);

						editor.set_block(
								LADDER, x + 2, 2, z - 1, std::nullopt, std::nullopt);
						editor.set_block(
								LADDER, x + 2, 1, z - 1, std::nullopt, std::nullopt);
					} else if (rc >= 20 && rc < 30) {
						// Sandpit
			...
			if (leisure_type == "pitch") {
				const std::vector<Block> vegetation{GRASS, FERN, RED_FLOWER,
						YELLOW_FLOWER, BLUE_FLOWER, WHITE_FLOWER, OAK_LEAVES};
				for (const auto &[x, z] : filled_area)
					editor.set_block(AIR, x, 1, z, vegetation, std::nullopt);
				sport_pitches::draw_pitch_markings(
						editor, element, filled_area, block_type);
			}
		}
	}
}
	...
	if (leisure_it != rel.tags.end() && leisure_it->second == "park") {
		...
		for (const ProcessedMember &member : rel.members) {
			if (member.role == ProcessedMemberRole::Outer) {
				...
				generate_leisure(editor, way_with_rel_tags, args, flood_fill_cache,
