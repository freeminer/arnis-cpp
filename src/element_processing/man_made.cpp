        ...
        if (nodes.size() < 3) {
            ...
            for (int dx = -2; dx <= 2; ++dx) {
                for (int dz = -2; dz <= 2; ++dz)
                    footprint.cells.insert({footprint.center_x + dx, footprint.center_z + dz});
            }
            return footprint;
        }
...
private:
    static bool point_in_polygon(int px_i, int pz_i, const std::vector<std::pair<int, int>>& polygon) {
...
void generate_water_tower(WorldEditor& editor, const ProcessedElement& element, const Args& args) {
    ...
    const int leg_offset = std::max(1, static_cast<int>(std::round(footprint.radius * 0.85)));
        ...
        for (int tier_y = 5; tier_y < support_height; tier_y += 5) {
            for (std::size_t i = 1; i < nodes.size(); ++i) {
                ...
                for (const auto& [bx, by, bz] : points)
                    editor.set_block(SMOOTH_STONE, bx, by, bz, std::nullopt, std::nullopt);
            }
        }
    }
    ...
    for (int y = tank_base; y < tank_base + tank_height; ++y) {
        for (const auto& [cx, cz] : footprint.cells_in_disc(footprint.radius))
            editor.set_block_absolute(POLISHED_ANDESITE, cx, y, cz, std::nullopt, std::nullopt);
    }
    for (const auto& [cx, cz] : footprint.cells_in_disc(footprint.radius)) {
...
void generate_silo(WorldEditor& editor, const ProcessedElement& element, const Args& args) {
    ...
    for (const auto& [cx, cz] : footprint.cells_in_disc(footprint.radius))
        editor.set_block_absolute(SMOOTH_STONE_SLAB, cx, base + height, cz, std::nullopt, std::nullopt);
}

void generate_storage_tank(WorldEditor& editor, const ProcessedElement& element, const Args& args) {
    TankFootprint footprint = TankFootprint::from_element(element);
    if (footprint.cells.empty())
        return;

    const int default_height = std::max(static_cast<int>(std::round(footprint.radius * 1.2)), 6);
    const int max_height = static_cast<int>(footprint.radius * 1.5) + 4;
    const int height = std::min(read_height(element, default_height, args.scale), max_height);
...
void generate_tank_structure(WorldEditor& editor, const ProcessedElement& element, const Args& args) {
...
void generate_man_made(WorldEditor& editor, const ProcessedElement& element, const Args& args) {
...
void generate_man_made_nodes(WorldEditor& editor, const ProcessedNode& node, const Args& args) {
