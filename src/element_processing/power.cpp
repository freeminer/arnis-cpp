...
void generate_power(WorldEditor &editor, const ProcessedElement &element) {
    ...
    if (it_location != element.tags().end() && 
        (it_location->second == "underground" || it_location->second == "underwater")) {
        return;
    }
...
void generate_power_tower_impl(WorldEditor &editor, int x, int z, int height) {
    ...
    for (int y = 1; y <= height; ++y) {
        ...
        int current_width = base_width - static_cast<int>((base_width - top_width) * progress);
        ...
        for (const auto& corner : corners) {
            editor.set_block(IRON_BLOCK, corner.first, y, corner.second, std::nullopt, std::nullopt);
        }
        ...
        if (y % 5 == 0 && y < height - 2) {
            ...
            for (int dx = -current_width; dx <= current_width; ++dx) {
                editor.set_block(IRON_BLOCK, x + dx, y, z - current_width, std::nullopt, std::nullopt);
                editor.set_block(IRON_BLOCK, x + dx, y, z + current_width, std::nullopt, std::nullopt);
            }
            for (int dz = -current_width; dz <= current_width; ++dz) {
                editor.set_block(IRON_BLOCK, x - current_width, y, z + dz, std::nullopt, std::nullopt);
                editor.set_block(IRON_BLOCK, x + current_width, y, z + dz, std::nullopt, std::nullopt);
            }
        }
        ...
        if (y % 5 >= 1 && y % 5 <= 4 && y > 1 && y < height - 2) {
            int prev_width = base_width - static_cast<int>((base_width - top_width) * ((y - 1) / static_cast<float>(height)));
    ...
    for (int arm_offset : {-arm_length, arm_length}) {
        ...
        for (int dx = 0; dx <= arm_length; ++dx) {
            ...
            editor.set_block(IRON_BLOCK, arm_x, arm_height, z, std::nullopt, std::nullopt);
            // Add second arm perpendicular
            editor.set_block(IRON_BLOCK, x, arm_height, z + ((arm_offset < 0) ? -dx : dx), std::nullopt, std::nullopt);
        }
        ...
        editor.set_block(END_ROD, x, arm_height - 1, z + arm_offset, std::nullopt, std::nullopt);
    }
    ...
    if (lower_arm_height > 5) {
        ...
        for (int arm_offset : {-lower_arm_length, lower_arm_length}) {
            for (int dx = 0; dx <= lower_arm_length; ++dx) {
                int arm_x = (arm_offset < 0) ? x - dx : x + dx;
                editor.set_block(IRON_BLOCK, arm_x, lower_arm_height, z, std::nullopt, std::nullopt);
            }
            int end_x = (arm_offset < 0) ? x - lower_arm_length : x + lower_arm_length;
            editor.set_block(END_ROD, end_x, lower_arm_height - 1, z, std::nullopt, std::nullopt);
        }
    }
    ...
    for (int dx = -3; dx <= 3; ++dx) {
        for (int dz = -3; dz <= 3; ++dz) {
            editor.set_block(GRAY_CONCRETE, x + dx, 0, z + dz, std::nullopt, std::nullopt);
        }
    }
}
...
void generate_power_pole(WorldEditor &editor, const ProcessedElement &element) {
    ...
    generate_power_pole_impl(editor, first_node_it->x, first_node_it->z, height, pole_material);
}
...
void generate_power_pole_impl(WorldEditor &editor, int x, int z, int height, const std::string& pole_material) {
    ...
    editor.set_block(END_ROD, x + arm_length, height + 1, z, std::nullopt, std::nullopt);
    editor.set_block(END_ROD, x, height + 1, z, std::nullopt, std::nullopt); // Center insulator
}
...
void generate_power_line(WorldEditor &editor, const ProcessedWay &way) {
    ...
    for (size_t i = 1; i < way.nodes.size(); ++i) {
        ...
        std::vector<std::tuple<int, int, int>> line_points = bresenham_line(start.x, 0, start.z, end.x, 0, end.z);
        ...
        for (size_t idx = 0; idx < line_points.size(); ++idx) {
            ...
            double denom = (line_points.size() > 1) ? static_cast<double>(line_points.size() - 1) : 1.0;
            ...
            if (base_height >= 18) {
                ...
                if (std::abs(dx) >= std::abs(dz)) {
                    ...
                    editor.set_block(chain_block, lx, wire_y, lz + 1, std::nullopt, std::nullopt);
                    editor.set_block(chain_block, lx, wire_y, lz - 1, std::nullopt, std::nullopt);
                } else {
                    // Line runs along Z, offset in X
                    editor.set_block(chain_block, lx + 1, wire_y, lz, std::nullopt, std::nullopt);
                    editor.set_block(chain_block, lx - 1, wire_y, lz, std::nullopt, std::nullopt);
                }
            }
        }
