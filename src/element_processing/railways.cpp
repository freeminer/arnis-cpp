...
const std::size_t CATENARY_MAST_INTERVAL = 16;
const int CATENARY_MAST_OFFSET = 3;
const int CATENARY_WIRE_HEIGHT = 6;
		...
		const vector<tuple<int, int, int>> &points)
{
	...
	for (size_t i = 0; i < points.size(); ++i) {
		...
		if (std::abs(x2 - x1) == 1 && std::abs(z2 - z1) == 1) {
			optional<tuple<int, int, int>> look_ahead =
					(i + 2 < points.size())
							? optional<tuple<int, int, int>>(points[i + 2])
							: nullopt;
			optional<tuple<int, int, int>> look_behind =
					(i > 0) ? optional<tuple<int, int, int>>(points[i - 1]) : nullopt;
...
Block determine_rail_direction(const pair<int, int> &current,
		const optional<pair<int, int>> &prev, const optional<pair<int, int>> &next)
{
	int x = current.first;
	...
	if (prev && next) {
		...
		if (px == nx) {
			...
			if ((from_prev == pair<int, int>{-1, 0} &&
						to_next == pair<int, int>{0, -1}) ||
					(from_prev == pair<int, int>{0, -1} &&
							to_next == pair<int, int>{-1, 0})) {
...
Block determine_rail_with_slope(const pair<int, int> &current,
		const optional<pair<int, int>> &prev, const optional<pair<int, int>> &next,
		int prev_ground, int current_ground, int next_ground)
{
...
bool catenary_wanted(const ProcessedWay &way)
{
	if (way.tags.get("railway") != "rail")
		return false;
	const auto electrified = way.tags.get("electrified");
	if (electrified == "contact_line")
		return true;
	return electrified == "yes" &&
		   (way.tags.get("usage") == "main" || way.tags.get("usage") == "branch");
}

vector<pair<int, int>> build_smoothed_centerline(const ProcessedWay &way)
{
	...
	for (size_t i = 1; i < way.nodes.size(); ++i) {
		...
		for (const auto &[x, y, z] : line)
			if (out.empty() || out.back() != pair<int, int>{x, z})
...
bool mast_base_clear(int cx, int cz, int dx, int dz, const CoordinateBitmap &roads,
		const CoordinateBitmap &buildings, const CoordinateBitmap &rails)
{
	const int x = cx + dx * CATENARY_MAST_OFFSET, z = cz + dz * CATENARY_MAST_OFFSET;
	if (roads.contains(x, z) || buildings.contains(x, z))
		return false;
	for (int d = 1; d <= CATENARY_MAST_OFFSET + 2; ++d)
		if (rails.contains(cx + dx * d, cz + dz * d))
			return false;
	return true;
}

void generate_catenary(WorldEditor &editor, const vector<pair<int, int>> &points,
		const CoordinateBitmap &roads, const CoordinateBitmap &buildings,
		const CoordinateBitmap &rails)
{
	if (points.size() < 2)
		return;
	for (size_t i = 0; i < points.size(); ++i) {
		const auto [cx, cz] = points[i];
		const auto [tx, tz] = i + 1 < points.size()
									  ? pair<int, int>{points[i + 1].first - cx,
												points[i + 1].second - cz}
									  : pair<int, int>{cx - points[i - 1].first,
												cz - points[i - 1].second};
		const int dx = (tx > 0) - (tx < 0), dz = (tz > 0) - (tz < 0);
		const int wire_y = editor.get_ground_level(cx, cz) + CATENARY_WIRE_HEIGHT;
		editor.set_block_absolute(
				dx ? CHAIN_X : CHAIN_Z, cx, wire_y, cz, nullopt, nullopt);
		if (i % CATENARY_MAST_INTERVAL)
			continue;
		int sx = -dz, sz = dx;
		if (!mast_base_clear(cx, cz, sx, sz, roads, buildings, rails)) {
			sx = -sx;
			sz = -sz;
			if (!mast_base_clear(cx, cz, sx, sz, roads, buildings, rails))
				continue;
		}
		const int mx = cx + sx * CATENARY_MAST_OFFSET,
				  mz = cz + sz * CATENARY_MAST_OFFSET;
		...
		const bridge_styles::BridgeOutlineIndex &bridge_outlines)
{
	...
	needed += 1;
	const std::size_t raw_ramp =
			static_cast<std::size_t>(total * RAIL_BRIDGE_RAMP_FRACTION);
	const std::size_t ramp_length = std::max(
			needed, std::clamp(raw_ramp, RAIL_BRIDGE_RAMP_MIN, RAIL_BRIDGE_RAMP_MAX));
	...
	for (std::size_t tds = 0; tds < total; ++tds) {
		const int start_ramp_y =
				start_internal ? deck_y
							   : static_cast<int>(std::round(
										 start_ground +
										 (deck_y - start_ground) *
												 std::min(1.0f, static_cast<float>(tds) /
																		denom)));
		const std::size_t dist_from_end = last_idx > tds ? last_idx - tds : 0;
		const int end_ramp_y =
				end_internal ? deck_y
							 : static_cast<int>(std::round(
									   end_ground +
									   (deck_y - end_ground) *
											   std::min(1.0f,
													   static_cast<float>(dist_from_end) /
															   denom)));
		bridge_ys.push_back(
	...
	for (std::size_t i = 0; i < total; ++i) {
		...
		optional<pair<int, int>> prev_xz =
				i > 0 ? optional<pair<int, int>>(all_points[i - 1]) : nullopt;
		...
		editor.set_block_absolute(
				(i % 4) == 0 ? OAK_LOG : GRAVEL, bx, y, bz, nullopt, nullopt);
...
void carve_subway_interior(
		WorldEditor &editor, const vector<pair<int, int>> &subway_points)
{
...
void generate_roller_coaster(WorldEditor &editor, const ProcessedWay &element)
{
	...
	// Skip indoor
	if (auto it_indoor = element.tags.find("indoor");
			it_indoor != element.tags.end() && it_indoor->second == "yes") {
		return;
	}
