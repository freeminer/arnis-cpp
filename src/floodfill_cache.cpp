...
CoordinateBitmap::CoordinateBitmap(const XZBBox& xzbbox) {
    ...
    width_ = static_cast<size_t>(static_cast<int64_t>(xzbbox.max_x()) - static_cast<int64_t>(min_x_) + 1);
    height_ = static_cast<size_t>(static_cast<int64_t>(xzbbox.max_z()) - static_cast<int64_t>(min_z_) + 1);
...
std::pair<size_t, size_t> CoordinateBitmap::count_in_range(int32_t min_x_range, int32_t min_z_range, int32_t max_x_range, int32_t max_z_range) const {
    ...
    for (int32_t z = min_z_range; z <= max_z_range; ++z) {
        ...
        if (local_z < 0 || local_z >= static_cast<int64_t>(height_)) {
            ...
            total_count += static_cast<size_t>(static_cast<int64_t>(max_x_range) - static_cast<int64_t>(min_x_range) + 1);
            continue;
        }
        size_t local_z_sz = static_cast<size_t>(local_z);

        // Calculate x range in local coordinates
        int64_t local_min_x = std::max(static_cast<int64_t>(min_x_range) - static_cast<int64_t>(min_x_), static_cast<int64_t>(0));
        int64_t local_max_x = std::min(static_cast<int64_t>(max_x_range) - static_cast<int64_t>(min_x_), static_cast<int64_t>(width_) - 1);

        // Count out-of-bounds x coordinates toward total
        int64_t x_start_offset = std::max(static_cast<int64_t>(min_x_) - static_cast<int64_t>(min_x_range), static_cast<int64_t>(0));
        int64_t x_end_offset = std::max(static_cast<int64_t>(max_x_range) - static_cast<int64_t>(min_x_) - (static_cast<int64_t>(width_) - 1), static_cast<int64_t>(0));
        ...
        if (start_byte == end_byte) {
            ...
            uint8_t mask = (num_bits_in_mask >= 8) ? 0xFFu : static_cast<uint8_t>((1U << num_bits_in_mask) - 1);
            ...
            uint8_t first_mask = static_cast<uint8_t>(~((1U << start_bit_in_byte) - 1)); // bits from start_bit to 7
            urban_count += static_cast<size_t>(__builtin_popcount(static_cast<uint8_t>(first_byte & first_mask)));
            ...
            uint8_t last_mask = (end_bit_in_byte >= 7) ? 0xFFu : static_cast<uint8_t>((1U << (end_bit_in_byte + 1)) - 1);
            urban_count += static_cast<size_t>(__builtin_popcount(static_cast<uint8_t>(last_byte & last_mask)));
        }
    }
...
bool FloodFillCache::way_needs_flood_fill(const ProcessedWay& way) {
    ...
    auto it_boundary = way.tags.find("boundary");
    ...
    bool has_boundary = (it_boundary != way.tags.end());
    ...
    return has_building || has_building_part || has_boundary || has_landuse || has_leisure || 
           has_amenity || natural_not_tree || (has_highway && area_yes) || tomb_pyramid;
}

std::optional<std::pair<int32_t, int32_t>> FloodFillCache::compute_centroid(const std::vector<std::pair<int32_t, int32_t>>& coords) {
    if (coords.empty()) {
        return std::nullopt;
    }
    ...
    return std::make_optional(std::make_pair(
        static_cast<int32_t>(sum_x / len),
        static_cast<int32_t>(sum_z / len)
    ));
}
...
FloodFillCache FloodFillCache::precompute(const std::vector<ProcessedElement>& elements, 
    ...
    for (const ProcessedWay* way : ways_needing_fill) {
        ...
        std::vector<std::pair<int32_t, int32_t>> filled = flood_fill_area(polygon_coords, timeout);
...
BuildingFootprintBitmap FloodFillCache::collect_building_footprints(
    ...
    for (const auto& element : elements) {
        if (element.is_way()) {
            ...
            bool is_building = (it_building != rel.tags.end()) || 
            ...
            if (is_building) {
                ...
                for (const auto& member : rel.members) {
                    if (member.role == ProcessedMemberRole::Outer) {
                        auto it = way_cache.find(member.way.id);
                        if (it != way_cache.end()) {
                            all_coords.insert(all_coords.end(), it->second.begin(), it->second.end());
                        }
                    }
                }
...
void configure_thread_pool(double cpu_fraction) {
    // In C++, we don't have Rayon, so this is a placeholder
    // In a real implementation, this might configure a custom thread pool
    // For now, we'll just use std::thread where needed
    (void)cpu_fraction; // Suppress unused parameter warning
}
