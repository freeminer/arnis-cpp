...
public:
	CoordinateBitmap() : min_x_(0), min_z_(0), width_(0), height_(0), count_(0) {}
	...
	/// Clears a coordinate, keeping the population count consistent.
	void clear(int32_t x, int32_t z);
	...
	std::pair<size_t, size_t> count_in_range(
			int32_t min_x, int32_t min_z, int32_t max_x, int32_t max_z) const;
};
...
private:
	...
	static std::optional<std::pair<int32_t, int32_t>> compute_centroid(
