	...
	auto parse_tags = [](const nlohmann::json &element) {
		...
		if (element.contains("tags") && element["tags"].is_object())
			for (auto it = element["tags"].begin(); it != element["tags"].end(); ++it)
				if (it.value().is_string()) tags[it.key()] = it.value().get<std::string>();
		return tags;
	};
	for (const auto &element : root["elements"]) {
		if (!element.is_object() || !element.contains("type") || !element["type"].is_string() ||
				!element.contains("id") || !element["id"].is_number_integer())
			continue;
		const auto type = element["type"].get<std::string>();
		const auto id = element["id"].get<std::int64_t>();
		if (type == "node") {
			if (!element.contains("lat") || !element.contains("lon") || !element["lat"].is_number() || !element["lon"].is_number()) continue;
			document.nodes.push_back({id, element["lat"].get<double>(), element["lon"].get<double>(), parse_tags(element)});
		} else if (type == "way") {
			RawWay way; way.id = id; way.tags = parse_tags(element);
			if (element.contains("nodes") && element["nodes"].is_array())
				for (const auto &node : element["nodes"])
					if (node.is_number_integer()) way.node_refs.push_back(node.get<std::int64_t>());
			document.ways.push_back(std::move(way));
		} else if (type == "relation") {
			RawRelation relation; relation.id = id; relation.tags = parse_tags(element);
			if (element.contains("members") && element["members"].is_array())
				for (const auto &value : element["members"]) {
					if (!value.is_object()) continue;
					RawRelationMember member;
					if (value.contains("type") && value["type"].is_string()) member.type = value["type"].get<std::string>();
					if (value.contains("ref") && value["ref"].is_number_integer()) member.ref = value["ref"].get<std::int64_t>();
					if (value.contains("role") && value["role"].is_string()) member.role = value["role"].get<std::string>();
					relation.members.push_back(std::move(member));
				}
			document.relations.push_back(std::move(relation));
		}
