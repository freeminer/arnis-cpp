...
std::string overpass_query(const geographic::LLBBox &bbox)
{
	...
	out << "[out:json][timeout:360][bbox:" << bbox.min().lat() << ',' << bbox.min().lng() << ',' << bbox.max().lat() << ',' << bbox.max().lng() << "];\n(\n"
...
{
	std::vector<OverpassEndpoint> official{{"https://overpass-api.de/api/interpreter"}, {"https://lz4.overpass-api.de/api/interpreter"}, {"https://z.overpass-api.de/api/interpreter"}};
	std::vector<OverpassEndpoint> fallback{{"https://maps.mail.ru/osm/tools/overpass/api/interpreter", true, 360}, {"https://overpass.private.coffee/api/interpreter", true, 120}};
		...
		const OverpassFetcher &fetcher, std::uint64_t seed, bool probe)
{
	...
	for (const auto &endpoint : overpass_request_plan(seed, probe)) {
		...
		try {
			...
			return FetchResult{osm_parser::parse_overpass_json(input), endpoint.url};
