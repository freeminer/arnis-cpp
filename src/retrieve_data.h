...
struct OverpassEndpoint
{
	std::string url;
	bool fallback = false;
	unsigned timeout_seconds = 360;
};
struct FetchResult
{
	osm_parser::RawOsmDocument document;
	std::string endpoint;
};
using OverpassFetcher = std::function<std::optional<std::string>(
		const OverpassEndpoint &, const std::string &)>;
